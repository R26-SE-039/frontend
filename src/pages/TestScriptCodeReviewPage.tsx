...
export default function TestScriptCodeReviewPage() {
  ...
  const handleGenerate = async () => {
    ...
    const selectedSuite = suites.find((suite) => suite.selected_for_run);

    if (suites.length > 0 && !selectedSuite) {
      setError('Select one suite with "Run this suite" before regenerating.');
      return;
    }
    ...
    setError(null);
    try {
      const frameworksToGenerate = selectedSuite
        ? [selectedSuite.framework]
        : setup.frameworks;

      await testCaseApi.generateTestCode(
        projectId,
        setup.url,
        setup.mode,
    ...
    <div className="space-y-8">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-[var(--muted)] mt-1">
            Stage 5 — AI generates one executable suite per framework. Review,
            edit, and pick the suite CI/CD should run.
            {setup && (
      ...
      {generating && (
        <div className="flex items-center gap-3 rounded-xl border border-indigo-200 bg-indigo-50 px-4 py-3 text-sm font-medium text-indigo-700">
          ...
          {suites.length > 0
            ? "Regenerating selected suite — this can take a minute."
      ...
      {staleCount > 0 && !generating && (
        <div className="flex items-center gap-3 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm font-medium text-amber-700">
          <AlertTriangle size={18} className="shrink-0" />
          {staleCount} suite{staleCount > 1 ? "s are" : " is"} stale — the
          Gherkin or DOM inputs changed since generation. Regenerate to stay in
          sync.
        </div>
      )}
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center gap-3 rounded-2xl border border-dashed border-slate-200 bg-white px-6 py-16 text-center">
          ...
          <p className="text-sm font-bold text-slate-900">
            No suites generated yet
          </p>
          <p className="max-w-sm text-xs font-medium text-slate-400">
            Approve Gherkin scenarios, finish the mode setup, and generate
            executable code for your frameworks.
          </p>
        </div>
      ) : (
        <div className="grid gap-6 lg:grid-cols-3">
          {suites.map((suite) => (
            <div
              key={suite.id}
              className="flex flex-col rounded-2xl border border-[var(--border)] bg-[var(--card)] shadow-sm overflow-hidden transition-all hover:-translate-y-0.5 hover:shadow-md"
            >
              <div className="border-b border-slate-100 p-5 space-y-3">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h3 className="text-sm font-black text-slate-900">
                        {frameworkLabels[suite.framework] ?? suite.framework}
                      </h3>
                ...
                <div className="flex flex-wrap items-center gap-2">
                  <TestCasePill
                    label={suite.mode === "dom" ? "DOM-aware" : "Abstract"}
                    type="source"
                  />
                  <span className="text-[10px] font-bold text-slate-400">
                    {suite.source_scenario_count} scenarios
                  </span>
                  {suite.llm_model && (
                    <span className="text-[10px] font-bold text-slate-400">
                      · {suite.llm_model}
              ...
              <div className="flex items-center justify-between gap-2 border-t border-slate-100 p-4">
                ...
                <button
                  type="button"
                  onClick={() =>
                    navigate(`/test-script/code-review/${suite.id}`)
                  }
