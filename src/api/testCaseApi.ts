...
async function request<T>(
  ...
  try {
    ...
    if (error instanceof DOMException && error.name === "AbortError") {
      throw new Error(
        `Test case service timed out after ${timeoutMs / 1000}s. Is the backend running on port 8002?`,
      );
    }
    throw new Error(
...
export const testCaseApi = {
  // ── Projects ──────────────────────────────────────────────────────────────
  listProjects: async (): Promise<TestCaseProject[]> =>
  ...
  ensureProject: async (): Promise<string> => {
    ...
    if (!current)
      throw new Error("Select a project before opening Test Case Gen.");
  ...
  deleteStory: async (projectId: string, storyId: string): Promise<void> =>
    request<void>(`/api/v1/projects/${projectId}/stories/${storyId}`, {
      method: "DELETE",
    }),

  // ── Gherkin (S2) ──────────────────────────────────────────────────────────
  generateGherkin: async (
    projectId: string,
    storyIds: string[],
  ...
  approveGherkin: async (gherkinId: string): Promise<GherkinResult> =>
    request<GherkinResult>(`/api/v1/gherkin/${gherkinId}/approve`, {
      method: "PUT",
    }),

  regenerateGherkin: async (
    projectId: string,
    storyId: string,
  ): Promise<GherkinResult> =>
    request<GherkinResult>(
      `/api/v1/gherkin/${projectId}/${storyId}/regenerate`,
      {
        method: "POST",
        timeoutMs: LLM_REQUEST_TIMEOUT_MS,
      },
  ...
  addDomElement: async (
    payload: Omit<
      DomElement,
      "id" | "edited_by_qa" | "approved" | "updated_at"
    >,
  ): Promise<DomElement> =>
  ...
  updateDomElement: async (
    ...
    patch: Partial<
      Pick<
        DomElement,
        "selector" | "tag" | "text" | "attributes" | "role" | "approved"
      >
    >,
  ): Promise<DomElement> =>
  ...
  getTestSuites: async (projectId: string): Promise<TestSuite[]> =>
    request<TestSuite[]>(
      `/api/v1/code/suites?project_id=${encodeURIComponent(projectId)}`,
    ),
  ...
  saveTestSuiteAsNewVersion: async (
    suiteId: string,
    code: string,
  ): Promise<TestSuite> =>
  ...
  restoreTestSuiteVersion: async (suiteId: string): Promise<TestSuite> =>
    request<TestSuite>(`/api/v1/code/suites/${suiteId}/restore`, {
      method: "POST",
    }),

  selectTestSuiteForRun: async (suiteId: string): Promise<TestSuite> =>
    request<TestSuite>(`/api/v1/code/suites/${suiteId}/select-for-run`, {
      method: "POST",
    }),
  ...
  pingGitHubConnection: async (
    projectId: string,
  ): Promise<GitHubPingResponse> =>
    request<GitHubPingResponse>(
  ...
  getRun: async (runId: string): Promise<RunDetail> =>
    request<RunDetail>(`/api/v1/runs/${runId}`),
...
export function openCrawlLogStream(
  ...
  const ws = new WebSocket(
    `${TEST_CASE_WS_URL}/ws/dom/crawl/${encodeURIComponent(runId)}`,
  );
  ws.onmessage = (event) => {
    try {
...
export function openExecutionStream(
  ...
  const ws = new WebSocket(
    `${TEST_CASE_WS_URL}/ws/execution/${encodeURIComponent(runId)}`,
  );
  ws.onmessage = (event) => {
    try {
...
export function openAgentEventStream(
  ...
  const ws = new WebSocket(
    `${TEST_CASE_WS_URL}/ws/agent/${encodeURIComponent(runId)}`,
  );
  ws.onmessage = (event) => {
    try {
...
export const runLogUrl = (runId: string): string =>
  `${TEST_CASE_API_URL}/api/v1/runs/${runId}/log`;

export const runPdfUrl = (runId: string): string =>
  `${TEST_CASE_API_URL}/api/v1/runs/${runId}/report.pdf`;
