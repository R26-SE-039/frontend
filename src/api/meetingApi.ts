...
export const meetingApi = {
  ...
  getChatHistory: async (meetingId: string) => {
    const response = await fetch(`${API_BASE_URL}/meeting/${meetingId}/chats`);
    if (!response.ok) throw new Error('Failed to fetch chat history');
    return response.json();
  },

  getTranscript: async (meetingId: string) => {
    const response = await fetch(`${API_BASE_URL}/meeting/${meetingId}/transcript`);
    if (!response.ok) throw new Error('Failed to fetch transcript');
    return response.json();
  },

  analyzeMeeting: async (meetingId: string, type: 'action_items' | 'summary' = 'action_items') => {
    const response = await fetch(`${API_BASE_URL}/meeting/${meetingId}/analyze`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
