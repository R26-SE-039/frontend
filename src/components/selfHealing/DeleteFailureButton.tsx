...
type DeleteFailureButtonProps = {
  testId: string;
  onDeleted?: () => void;
};

export default function DeleteFailureButton({ testId, onDeleted }: DeleteFailureButtonProps) {
  ...
  const handleDelete = async () => {
    ...
    setLoading(true);
    try {
      await failureAnalysisApi.deleteFailure(testId);
      onDeleted?.();
    } catch (error) {
      console.error(error);
